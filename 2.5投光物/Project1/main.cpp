...
{	glfwInit();	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR,3);
	...
	Material material = Material(testshader,
		...
		64.0f);
	/*LightPoint light =LightPoint (testshader,
		glm::vec3(1.2f,1.0f,2.0f),
		glm::vec3(1.0f,1.0f,1.0f),
		glm::vec3(1.0f,1.0f,1.0f),
		glm::vec3(1.0f,1.0f,1.0f),1.0f,0.09f,0.032f);*/
	LightSpot light = LightSpot(testshader,
		camera.Position,
		camera.Forward,
		glm::vec3(0.5f,0.5f,0.5f),
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			glUniform3f(glGetUniformLocation(testshader.ID, "light.direction"), light.direction.x, light.direction.y, light.direction.z);
			...
			glUniform1f(glGetUniformLocation(testshader.ID, "light.cutoff"), light.cutoff);
			glUniform1f(glGetUniformLocation(testshader.ID, "light.outercutoff"), light.outercutoff);
			glUniform1f(glGetUniformLocation(testshader.ID, "light.constant"), light.constant);
			glUniform1f(glGetUniformLocation(testshader.ID, "light.linear"), light.linear);
			glUniform1f(glGetUniformLocation(testshader.ID, "light.quadratic"), light.quadratic);
