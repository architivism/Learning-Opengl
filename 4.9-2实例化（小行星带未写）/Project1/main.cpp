...
{	
	...
	Shader explodeshader = Shader("VertexExplode.vert", "fragmentSource.frag","geometry.geom");
	Shader normalshader = Shader("vertexSourceNormal.vert", "fragmentSource1.frag", "geometryNormal.geom");
...
#pragma region Init and Load Models to VAO,VBO
	...
	Model model_planet(exePath.substr(0, exePath.find_last_of('\\')) + "\\planet\\planet.obj");
	Model model_rock(exePath.substr(0, exePath.find_last_of('\\')) + "\\rock\\rock.obj");
...
#pragma region MVP matrices
	unsigned int uniformBlockIndexcontainer = glGetUniformBlockIndex(testshader.ID, "Matrices");
	unsigned int uniformBlockIndexskyRef = glGetUniformBlockIndex(skyboxreflect.ID, "Matrices");
	unsigned int uniformBlockIndexglass = glGetUniformBlockIndex(grassshader.ID, "Matrices");
	...
	glUniformBlockBinding(testshader.ID, uniformBlockIndexcontainer, 0);
	glUniformBlockBinding(skyboxreflect.ID, uniformBlockIndexskyRef, 0);
	glUniformBlockBinding(grassshader.ID, uniformBlockIndexglass, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 11; i++)
		{
			if (i == 10) {
				...
				model_planet.Draw(testshader);
				model_rock.Draw(testshader);
		...
		//画分解盒子
		explodeshader.use();
		model = glm::translate(glm::mat4(1.0f),glm::vec3(0,3.0,-5.0));
		...
		glUniform3f(glGetUniformLocation(explodeshader.ID, "material.ambient"), material.ambient.x, material.ambient.y, material.ambient.z);
		glUniform3f(glGetUniformLocation(explodeshader.ID, "lightD.direction"), lightD.direction.x, lightD.direction.y, lightD.direction.z);
		glUniform3f(glGetUniformLocation(explodeshader.ID, "lightD.ambient"), lightD.ambient.x, lightD.ambient.y, lightD.ambient.z);
		glUniform3f(glGetUniformLocation(explodeshader.ID, "lightD.diffuse"), lightD.diffuse.x, lightD.diffuse.y, lightD.diffuse.z);
		glUniform3f(glGetUniformLocation(explodeshader.ID, "lightD.specular"), lightD.specular.x, lightD.specular.y, lightD.specular.z);

		cube.DrawArray(explodeshader, material.diffuse, material.specular);
		...
		glUniformMatrix4fv(glGetUniformLocation(skyboxshader.ID, "view"), 1, GL_FALSE, glm::value_ptr(skybox_view));
