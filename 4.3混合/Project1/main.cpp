...
{	glfwInit();	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR,3);
	...
	Shader grassshader=Shader("vertexSource.vert","fragmentSource_glass.frag");
	Material material = Material(testshader,
		...
		64.0f);
	Material material_glass(grassshader,
		glm::vec3(0.5,0.5,0.5),
		Load_pic_to_GPU("grass.png", GL_RGBA, GL_RGBA, 0),
		Load_pic_to_GPU("grass.png", GL_RGBA, GL_RGBA, 0),
		32.0
	);
	...
	unsigned int binderVAO;
	glGenVertexArrays(1,&binderVAO);
	glBindVertexArray(binderVAO);
		;
    glBindBuffer(GL_ARRAY_BUFFER,VBO);
